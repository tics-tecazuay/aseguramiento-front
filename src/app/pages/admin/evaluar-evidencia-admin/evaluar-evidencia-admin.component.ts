...
@Component({
  selector: 'app-evaluar-evidencia-admin',
  templateUrl: './evaluar-evidencia-admin.component.html',
  styleUrls: ['./evaluar-evidencia-admin.component.css']
})
export class EvaluarEvidenciaAdminComponent implements OnInit {
  ...
  onSelectionChange(event: MatSelectionListChange) {
    ...
    console.log('Opción seleccionada:', event.options[0].value);
    ...
    this.usernameResponsable = this.usuarioSeleccionado.username;
  ...
  verDetalles(element: any) {
    ...
    this.descripcionDeta = element.descripcionevidencia;
    console.log("ID ASIGNACION_EVIDENCIA " + this.idasigna);
    this.actividadService.getEviAsig(this.idasigna, this.id_modelo).subscribe((data) => {
  ...
  setEvidenciaRechazada(element: any) {
    ...
    this.actividadService.getEviAsig(this.idasigna, this.id_modelo)
  ...
  guardarFechaPlazo() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.isLoading = true;
        this.actividadService.getEviAsig(this.idasigna, this.id_modelo).subscribe((asignaciones) => {
          if (asignaciones && asignaciones.length > 0) {
  ...
  guardarCambioUsuario(): void {
    ...
    this.isLoading = true;
    this.asignarEvidenciaService.cambiarUsuario(this.idasigna, this.selectedUser.id, this.id_modelo).subscribe(
      (response: any) => {
